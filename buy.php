...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['confirm_payment'])) {
        ...
        $success = true;
        $generated_username = $_SESSION['generated_username'] ?? "";
        ...
        // Hapus data dari database jika pembelian dibatalkan
        ...
        if (empty($voucher_price) || empty($payment_method) || empty($email) || empty($whatsapp)) {
            ...
            // Simpan username dan password di sesi
            ...
            if ($payment_method === 'transfer') {
                ...
                $qris_image = "img/qris_barcode.png"; // Nama file gambar QRIS
            }

            if (empty($error)) {
                // Simpan ke database
                $stmt = $conn->prepare("INSERT INTO pembelian (voucher_name, voucher_price, email, whatsapp_number, payment_method, username, password, start_time, end_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("sdsssssss", $voucher_name, $voucher_price, $email, $whatsapp, $payment_method, $generated_username, $generated_password, $start_time_formatted, $end_time_formatted);
